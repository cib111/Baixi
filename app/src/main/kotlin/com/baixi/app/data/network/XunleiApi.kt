...
class XunleiApi(
    ...
    /** 临时转存目录名提供者（设置页可配置，动态生效）；默认对齐迅雷常量 */
    var tempDirNameProvider: () -> String = { XunleiConstants.TEMP_DIR_NAME }
    ...
    suspend fun ensureTempDir(
        ...
        val dirName = tempDirNameProvider()
        val root = getFiles("", accessToken, deviceId, captchaToken) ?: emptyList()
        root.firstOrNull { it.isdir && it.fname == dirName }?.fid
            ?: createFolder(dirName, "", accessToken, deviceId, captchaToken)
    }
