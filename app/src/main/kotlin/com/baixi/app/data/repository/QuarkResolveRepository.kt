 ...
 */
class QuarkResolveRepository(
    private val api: QuarkApi,
    /** 临时转存目录名提供者（设置页可配置，动态生效） */
    private val tempDirNameProvider: () -> String = { QuarkConstants.TEMP_DIR_NAME }
    ...
    /**
     * 确保「Baixi临时转存」目录存在，返回其 fid；不存在则创建。
     */
    override suspend fun ensureTempDir(cookie: String): Result<String> = runCatching {
        ...
        rootFiles.firstOrNull { it.isdir && it.fname == tempDirName }?.fid
            ?: api.createFolder(tempDirName, QuarkConstants.DEFAULT_PDIR_FID, cookie)
     ...
     *  1) 每次转存落到「Baixi临时转存」下的【唯一子目录 tr_<时间戳>_<随机>】，
    ...
    override suspend fun getShareDownloadLink(
        ...
        val subDirName = "${QuarkConstants.TEMP_SUBDIR_PREFIX}${System.nanoTime()}_${(Math.random() * 1_000_000).toInt()}"
        val subDirFid = api.createFolder(subDirName, baseDir, cookie)
            ?: throw IllegalStateException("创建临时转存目录失败")

        // 「转存 + 取链」全过程包在 try 中：任一步失败都必须回滚删除临时子目录，
        // 否则 tr_* 目录（含完整转存文件）会永久残留在用户网盘里占用空间
        try {
            ...
            // 失败回滚：删除临时子目录（连同其中已转存的文件）；清理本身失败不掩盖原始异常
            runCatching { api.deleteFile(subDirFid, cookie) }
            throw t
        }
    }.fold(
        onSuccess = { Result.success(it) },
    ...
    /** 查找「Baixi临时转存」目录 fid（分页扫描根目录，最多 10 页）；未找到返回 null */
    private suspend fun findTempDirFid(cookie: String): String? {
        val tempDirName = tempDirNameProvider()
        var page = 1
        while (page <= 10) {
            val batch = api.getFileList(QuarkConstants.DEFAULT_PDIR_FID, cookie, page, 100)
                ?: return null
            batch.firstOrNull { it.isdir && it.fname == tempDirName }?.let { return it.fid }
    ...
    /**
     * 清理历史遗留的临时转存子目录（供应用启动时调用，由 MainScreen 接线）：
     * 列出「Baixi临时转存」下所有以 QuarkConstants.TEMP_SUBDIR_PREFIX（"tr_"）开头的目录并逐个删除。
     * 用途：修复旧版本「取链失败未回滚」遗留的孤儿目录，避免永久占用用户夸克网盘空间。
     * 全过程失败不抛异常（启动路径不应因清理失败而中断）。
     */
    suspend fun cleanupOrphanTempDirs(cookie: String) {
        runCatching {
            val baseDirFid = findTempDirFid(cookie) ?: return@runCatching
            var page = 1
            while (page <= 10) {
                val children = api.getFileList(baseDirFid, cookie, page, 100).orEmpty()
                for (child in children) {
                    if (child.isdir && child.fname.startsWith(QuarkConstants.TEMP_SUBDIR_PREFIX)) {
                        // 单个目录删除失败不影响其余目录
                        runCatching { api.deleteFile(child.fid, cookie) }
                    }
                }
                if (children.size < 100) break
                page++
            }
        }
    }
