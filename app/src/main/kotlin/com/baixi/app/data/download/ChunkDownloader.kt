...
class ChunkDownloader(private val clientProvider: () -> OkHttpClient) {
    ...
    /**
     * 读取服务器文件校验标识（ETag / Last-Modified），用于断点续传前判断服务器文件是否已变化。
     * 与大小探测同一请求形式（Range bytes=0-0 的 GET）：部分网盘 CDN 不支持 HEAD。
     * @return Pair(etag, lastModified)；两者都拿不到时返回 null
     */
    suspend fun getValidators(url: String, headers: Map<String, String>): Pair<String?, String?>? =
            ...
            try {
                runCatching {
                    call.execute().use { response ->
                        ...
                        if (etag == null && lastModified == null) null else etag to lastModified
                    }
                }.getOrNull()
            } finally {
        ...
        try {
            return call.execute().use { response ->
                ...
                when (val code = response.code) {
                    ...
                    200 -> {
                        // ★ 200 的响应体永远从文件第 0 字节开始，而本分片要写的位置是 partFile 偏移 existing
                        //   （对应文件字节 from = start + existing）。因此只有「区间起点为 0 且尚未写入」
                        //   （from == 0 && existing == 0）时，取响应体前 expected 字节才正好是本区间内容；
                        //   其余情况（含旧实现按 Content-Length 猜测「200 当 206」的分支）续写都会造成
                        //   前缀重复/内容错位 → 一律返回 RANGE_IGNORED，由上层回退单流整文件（写全新文件）。
                        if (unknownTotal || existing > 0L || from > 0L) {
                            Log.w(
                                TAG,
                                "downloadChunk: task=$taskId range=$from-$end 返回200但起点非0 → 回退完整GET"
                            )
                            ChunkResult.RANGE_IGNORED
                        } else {
                            val body = response.body ?: return@use ChunkResult.FAILED
                            val expectedLen = end - from + 1
                            val written = writeSlice(body.byteStream(), partFile, 0L, expectedLen, onBytes)
                            if (written != expectedLen) {
                                Log.w(TAG, "downloadChunk: task=$taskId 200(起点0)写入不足 written=$written 预期=$expectedLen")
                                ChunkResult.FAILED
                            } else {
                                ChunkResult.OK
                            }
            ...
            // ★ 收尾时若该任务已无在途请求，连 key 一起移除：旧实现只在 cancelCalls 里删 key，
            //   正常完成的任务会留下空集合常驻（长会话下缓慢泄漏）。
            val remaining = activeCalls[taskId]
            remaining?.remove(call)
            if (remaining != null && remaining.isEmpty()) activeCalls.remove(taskId, remaining)
            cancelHandle?.dispose()
        }
    }
