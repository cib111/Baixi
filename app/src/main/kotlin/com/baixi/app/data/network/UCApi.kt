...
import com.baixi.app.data.network.model.PlayLink
import com.baixi.app.data.network.model.QuotaInfo
import com.baixi.app.data.network.model.ShareFile
import com.baixi.app.data.network.model.ShareInfo
import com.baixi.app.data.network.model.ShareToken
    ...
    suspend fun refreshSession(cookie: String): String? = withContext(Dispatchers.IO) {
        ...
        runCatching {
            client.newCall(request).execute().use { resp ->
                ...
                // 只有「HTTP 成功 且 服务端确实下发了新 Cookie」才算刷新成功：
                // 否则返回 null（调用方据此不推进刷新计时，下次仍可重试），避免谎报成功
                if (!resp.isSuccessful || merged == cookie) return@use null
                cookieSink?.invoke(merged)
                merged
            }
        }.getOrNull()
    }
