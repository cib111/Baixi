 ...
 */
class UCResolveRepository(
    private val api: UCApi,
    /** 临时转存目录名提供者（设置页可配置，动态生效） */
    private val tempDirNameProvider: () -> String = { UCConstants.TEMP_DIR_NAME }
    ...
    override suspend fun ensureTempDir(cookie: String): Result<String> = runCatching {
        ...
        rootFiles.firstOrNull { it.isdir && it.fname == tempDirName }?.fid
            ?: api.createFolder(tempDirName, UCConstants.DEFAULT_PDIR_FID, cookie)
