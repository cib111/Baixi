...
class Pan123Api(
    ...
    private fun executeJson(request: Request): JSONObject {
        client.newCall(request).execute().use { response ->
            ...
            // 非 2xx 或响应体为空都视为失败（原写法用 && 会漏判：2xx 空体、非 2xx 非空体都会被放过）
            if (!response.isSuccessful || body.isBlank()) {
                throw IllegalStateException("请求失败（HTTP ${response.code}）")
            }
            // JSON 解析失败统一包装为 IllegalStateException（带 HTTP 码），避免未包装的 JSONException 冒泡
            return runCatching { JSONObject(body) }.getOrElse {
                throw IllegalStateException("响应解析失败（HTTP ${response.code}）")
            }
        }
    }
}
