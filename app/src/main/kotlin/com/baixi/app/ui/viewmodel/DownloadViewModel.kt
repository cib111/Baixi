package com.baixi.app.ui.viewmodel

import androidx.lifecycle.ViewModel
import androidx.lifecycle.ViewModelProvider
import androidx.lifecycle.viewModelScope
import com.baixi.app.data.db.DownloadTaskEntity
import com.baixi.app.data.download.DownloadManager
import com.baixi.app.data.download.DownloadStats
import kotlinx.coroutines.flow.SharingStarted
import kotlinx.coroutines.flow.StateFlow
import kotlinx.coroutines.flow.stateIn
import kotlinx.coroutines.launch

/**
 * 下载页 ViewModel：任务列表（Room Flow → StateFlow）+ 实时统计 + 操作转发。
 */
class DownloadViewModel(private val manager: DownloadManager) : ViewModel() {

    val tasks: StateFlow<List<DownloadTaskEntity>> = manager.tasks
        .stateIn(
            scope = viewModelScope,
            started = SharingStarted.WhileSubscribed(5000),
            initialValue = emptyList()
        )

    /** 实时下载统计：任务 id → 速度/剩余时间/线程数 */
    val stats: StateFlow<Map<Long, DownloadStats>> = manager.stats

    /** 添加下载任务（headers 可携带 Referer/Cookie 等） */
    fun enqueue(url: String, fileName: String, headers: Map<String, String> = emptyMap()) {
        viewModelScope.launch { manager.enqueue(url, fileName, headers) }
    }

    fun pause(id: Long) = manager.pause(id)

    /** 恢复单个任务（用户主动点击开始/继续/重试：跳过「等待WiFi」策略，立即下载） */
    fun resume(id: Long, ignoreMeteredWait: Boolean = false) = manager.start(id, ignoreMeteredWait = ignoreMeteredWait)

    /** 设置任务优先级（数值越大越先占用下载槽位） */
    fun setPriority(id: Long, priority: Int) {
        viewModelScope.launch { manager.setPriority(id, priority) }
    }

    /** 设置单任务线程数（0=跟随平台/全局设置）；下载中任务由 UI 侧拦截 */
    fun setThreadOverride(id: Long, threads: Int) {
        viewModelScope.launch { manager.setThreadOverride(id, threads) }
    }

    fun remove(id: Long, deleteLocal: Boolean = false) = manager.remove(id, deleteLocal)

    /** 全部暂停：暂停所有正在下载（含等待中/等待WiFi）的任务 */
    fun pauseAll() {
        tasks.value.filter {
            it.status == DownloadTaskEntity.STATUS_DOWNLOADING ||
                it.status == DownloadTaskEntity.STATUS_PENDING ||
                it.status == DownloadTaskEntity.STATUS_WAITING_WIFI
        }.forEach { manager.pause(it.id) }
    }

    /** 全部开始：恢复所有已暂停/失败/等待WiFi的任务（断点续传；等待WiFi的任务若仍在流量网络会再次询问/等待） */
    fun resumeAll() {
        tasks.value.filter {
            it.status == DownloadTaskEntity.STATUS_PAUSED ||
                it.status == DownloadTaskEntity.STATUS_FAILED ||
                it.status == DownloadTaskEntity.STATUS_WAITING_WIFI
        }.forEach { manager.start(it.id) }
    }

    /** 删除全部任务（可同时删除已保存到本地的文件） */
    fun removeAll(deleteLocal: Boolean = false) {
        tasks.value.toList().forEach { manager.remove(it.id, deleteLocal) }
    }

    class Factory(private val manager: DownloadManager) : ViewModelProvider.Factory {
        @Suppress("UNCHECKED_CAST")
        override fun <T : ViewModel> create(modelClass: Class<T>): T {
            require(modelClass.isAssignableFrom(DownloadViewModel::class.java))
            return DownloadViewModel(manager) as T
        }
    }
}