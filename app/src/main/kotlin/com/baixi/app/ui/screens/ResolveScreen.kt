...
import com.baixi.app.ui.resolve.DownloadLinkDialog
import com.baixi.app.ui.resolve.ShareDetailScreen
...
import com.baixi.app.ui.viewmodel.ResolveUiState
...
import com.baixi.app.util.QrDecoder
import com.baixi.app.util.AppVisibility
...
fun ResolveScreen(
    ...
    // 设置仓库：读剪切板自动识别开关（prefs 进程级，每次读取均为最新值）
    ...
    // 相册二维码识别：选图 → IO 线程解码 → 从识别文本里提取分享链接
    // 1 条 → 填入输入框并解析；多条 → 交给批量解析；0 条 → Snackbar 提示
    val qrLauncher = rememberLauncherForActivityResult(
        ActivityResultContracts.GetContent()
    ) { uri ->
        if (uri == null) return@rememberLauncherForActivityResult
        scope.launch {
            val text = withContext(Dispatchers.IO) { QrDecoder.decode(context, uri) }
            val links = text?.let { extractShareLinks(it) }.orEmpty()
            when {
                links.isEmpty() -> SnackbarController.show("图片中未识别到分享链接")
                links.size == 1 -> {
                    val one = links[0]
                    val parsedPwd = ShareLinkParser.parse(one)?.pwd
                    link = one
                    pwd = parsedPwd.orEmpty()
                    pwdEdited = true
                    viewModel.startResolve(one, parsedPwd)
                }
                else -> {
                    link = links.joinToString("\n")
                    viewModel.startResolveBatch(links)
                }
            }
        }
    }

    // 外部入口（关于页/引导页「源码下载」）预填链接：以 pendingLink 为 key（解析页常驻组合，
    // 用 Unit 不会重触发），值变化即消费——填入输入框并自动解析；已有解析进行中则仅填入不触发
    LaunchedEffect(viewModel.pendingLink) {
        viewModel.consumePendingLink()?.takeIf { it.isNotBlank() }?.let { pending ->
            link = pending
            if (viewModel.uiState is ResolveUiState.Idle) {
                viewModel.startResolve(pending, "")
            }
        }
    }
    ...
    // 注意：此处必须实时读取 viewModel.uiState（外部可观察状态），不能按值捕获组合期的局部 state，
    // 否则闭包里的「仅在输入态(Idle)才提示」会冻结在首帧（DisposableEffect / 轮询只注册一次）。
    val maybeSuggestClipboard: () -> Unit = {
        if (!settingsRepo.clipboardAutoDetect) {
            // 设置页开关关闭：不再建议，并清掉已弹出的旧建议（切回本页时提示条消失）
    ...
    // 四重门禁：① 低版本 ② 设置项 clipboardAutoDetect 开启（关闭时连剪贴板都不读）
    // ③ 页面处于输入态（key 变化时重启/停止轮询，避免离开输入页后仍在后台读剪贴板）
    // ④ 应用处于前台（进后台立即停止读取，避免后台读剪贴板的隐私问题）
    val isInputState = state is ResolveUiState.Idle
    if (Build.VERSION.SDK_INT < Build.VERSION_CODES.S && settingsRepo.clipboardAutoDetect) {
        LaunchedEffect(isInputState) {
            if (isInputState) {
                while (true) {
                    kotlinx.coroutines.delay(2000)
                    // 前台门禁：应用不可见时一律不读剪贴板（AppVisibility 由 BaixiApp 安装）
                    if (AppVisibility.isForeground) maybeSuggestClipboard()
                }
            }
        }
    }
        ...
        AnimatedContent(
            ...
            scrollBehavior = scrollBehavior,
                ...
                is ResolveUiState.Loading -> LoadingContent()
                    ...
                    onClearPwd = { pwd = "" },
                    // B1：一次提交多条链接 → 批量解析；单条/识别不到 → 原逻辑（行为不变）
                    onResolve = {
                        val links = extractShareLinks(link)
                        if (links.size > 1) viewModel.startResolveBatch(links)
                        else viewModel.startResolve(link, pwd)
                    },
                    onPickQr = { qrLauncher.launch("image/*") }
                )
            }
        }
    ...
    // 批量解析中：输入页进度弹窗（详情页有自己的批量弹窗，用 state 判断避免两个弹窗同时出现）
    if (viewModel.isBatchWorking && state !is ResolveUiState.Detail) {
        AlertDialog(
            onDismissRequest = { },
            confirmButton = { },
            dismissButton = {
            ...
            text = {
                Row(verticalAlignment = Alignment.CenterVertically) {
                    ...
                    Text(
                        text = viewModel.batchProgress ?: "正在批量解析，请稍候…",
...
private fun ResolveInputContent(
    ...
    onClearLink: () -> Unit,
    onClearPwd: () -> Unit,
    /** 解析入口（可能是单条，也可能识别出多条走批量） */
    onResolve: () -> Unit,
    /** 从相册选二维码图片识别分享链接 */
    onPickQr: () -> Unit
) {
    val isLoading = state is ResolveUiState.Loading
    val isBatchWorking = viewModel.isBatchWorking
    ...
    Column(
        ...
        // 解析入口 + 二维码入口（二维码用「图标 + 文字」，不做相机预览扫描）
        Row(
            modifier = Modifier.fillMaxWidth(),
            ...
            Button(
                ...
                modifier = Modifier
                    .weight(1f)
                    .height(48.dp),
                enabled = link.isNotBlank() && !isLoading && !isBatchWorking
            ) {
                if (isLoading) {
                    CircularProgressIndicator(
            ...
            OutlinedButton(
                ...
                enabled = !isLoading && !isBatchWorking
            ) {
                Icon(
                    Icons.Outlined.Link,
...
/** 分享链接提取正则（与 ShareLinkParser 内部同款；多链接批量解析与二维码文本提取共用） */
private val SHARE_URL_REGEX = Regex("""https?://[^\s]+""")

/**
 * 从整段文案 / 二维码识别文本中提取全部可识别的网盘分享链接（B1 批量解析用）。
 * 每条 URL 逐条交给 ShareLinkParser.parse 过滤（非网盘链接被丢弃），去重并保持出现顺序。
 */
private fun extractShareLinks(text: String): List<String> =
    SHARE_URL_REGEX.findAll(text)
        .map { it.value.trimEnd('。', '，', ',', '；', ';', ')', ']', '}', '"', '\'') }
        .filter { it.isNotBlank() && ShareLinkParser.parse(it) != null }
        .distinct()
        .toList()
